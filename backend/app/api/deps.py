...
def get_current_warehouse_member(
    ...
    if x_warehouse_id is None:
        # No header: fall back to the user's first membership. Cross-tenant
        # auto-join (the old "SELF-HEAL" path) was removed because it silently
        # made callers admin of warehouse_id=1.
        member = (
            db.query(WarehouseMember)
            .filter(WarehouseMember.user_id == current_user.id)
            .first()
        )
        if not member:
